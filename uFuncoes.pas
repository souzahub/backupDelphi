...
function Preencher(Const Variavel: String; Const Tamanho: Integer;
  Const Conteudo: String): String;
function Alinhar(Const Variavel: String; Const Tamanho: Integer;
...
function AchaDVNossoNr(Const Agencia: String; Const TipoConta: String;
  Const NrConta: String; Const DVConta: String; NossoNr: String): String;
function AchaDVNossoNrCB(NossoNr: String): String;
function MontaChaveAcessoNFe(codUF: Integer; dtEmissao: TDateTime; CNPJ: String;
  Modelo, Serie, NumeroNF, TpEmis, Sequencial: Integer): String;
...
function AchaSaldoCleanAteFim(Conta: String; Origem: String;
  DTFim: TDateTime): real;
function AchaSaldoAte(Conta: String; Data: TDateTime): real;
function AchaSaldoAteSemFechamento(Conta: String; Data: TDateTime;
  vZerar: String): real;
...
function CaptureScreenRect(Canvas: TCanvas; HandleOrig: THandle; SrcRect: TRect;
  X, Y: Integer): HDC;
function RemoverAcento(Key: Char): Char;
function AchaSaldoAteFrota(Conta: String; Data: TDateTime): real;
function AchaSaldoPeriodoSemFechamento(Conta: String; Inicio: TDateTime;
  Fim: TDateTime): real;
function AchaSaldoPeriodoSemFechamentoClean(Conta: String; Origem: String;
  Inicio: TDateTime; Fim: TDateTime): real;
function AchaSaldoPeriodoSemFechamentoCleanSub(Conta: String; SubConta: String;
  Origem: String; SubOrigem: String; Inicio: TDateTime; Fim: TDateTime): real;
...
begin
  ...
  try
    BitBlt(Canvas.Handle, X, Y, SrcRect.Right - SrcRect.Left,
      SrcRect.Bottom - SrcRect.Top, ScreenDC, SrcRect.Left,
      SrcRect.Top, SRCCOPY);
  finally
...
begin
  ...
  chave := IntToStr(codUF) + copy(emissao, 7, 2) + copy(emissao, 4, 2) +
    ...
    + StrZero(IntToStr(NumeroNF), 9) + IntToStr(TpEmis) +
    StrZero(IntToStr(Sequencial), 8);
...
begin
  ...
  NossoNr := '09' + NossoNr;
  for X := 1 to 13 do
    SNossoNr := SNossoNr + (StrToInt(copy(NossoNr, X, 1)) *
      StrToInt(copy(PesoNossoNr, X, 1)));
...
function AchaDVNossoNr(Const Agencia: String; Const TipoConta: String;
  Const NrConta: String; Const DVConta: String; NossoNr: String): String;
var
  PesoAgencia, PesoTipoConta, PesoNrConta, PesoDVConta, PesoNossoNr: String;
...
begin
  ...
  SNossoNr := 0;
  for X := 1 to 4 do
    SAgencia := SAgencia + (StrToInt(copy(Agencia, X, 1)) *
      StrToInt(copy(PesoAgencia, X, 1)));
  for X := 1 to 2 do
    STipoConta := STipoConta + (StrToInt(copy(TipoConta, X, 1)) *
      StrToInt(copy(PesoTipoConta, X, 1)));
  for X := 1 to 7 do
    SNrConta := SNrConta + (StrToInt(copy(NrConta, X, 1)) *
      StrToInt(copy(PesoNrConta, X, 1)));
...
begin
  ...
  if Forma = 'D' then { Direita }
    ...
    Result := (copy(dados, 1, StrToInt(FormatFloat('#0', length(dados) / 2))) +
      Variavel + copy(dados, 1, StrToInt(FormatFloat('#0',
      length(dados) / 2))));
end;
...
begin
  ...
  Soma := StrToInt(TextCGC[01]) * 5 + StrToInt(TextCGC[02]) * 4 +
    StrToInt(TextCGC[03]) * 3 + StrToInt(TextCGC[04]) * 2 + StrToInt(TextCGC[05]
    ) * 9 + StrToInt(TextCGC[06]) * 8 + StrToInt(TextCGC[07]) * 7 +
    StrToInt(TextCGC[08]) * 6 + StrToInt(TextCGC[09]) * 5 + StrToInt(TextCGC[10]
    ) * 4 + StrToInt(TextCGC[11]) * 3 + StrToInt(TextCGC[12]) * 2;
  Digito1 := Soma mod 11;
  if Digito1 <= 1 then
    Digito1 := 0
  else
  ...
  Soma := StrToInt(TextCGC[01]) * 6 + StrToInt(TextCGC[02]) * 5 +
    StrToInt(TextCGC[03]) * 4 + StrToInt(TextCGC[04]) * 3 + StrToInt(TextCGC[05]
    ) * 2 + StrToInt(TextCGC[06]) * 9 + StrToInt(TextCGC[07]) * 8 +
    StrToInt(TextCGC[08]) * 7 + StrToInt(TextCGC[09]) * 6 + StrToInt(TextCGC[10]
    ) * 5 + StrToInt(TextCGC[11]) * 4 + StrToInt(TextCGC[12]) * 3 + Digito1 * 2;
  Digito2 := Soma mod 11;
  if Digito2 <= 1 then
    Digito2 := 0
  else
